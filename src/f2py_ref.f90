...
subroutine runtrj_obs_ead(q, p, qe, pe, bt, Et, Ead, rep, dt, ierr, nt, nf_, ns)
   use types
   use pes,  only : potad, grad_a, mass
   use mash, only : evolve, cstate, pops_phi
   integer, intent(in) :: nt, nf_, ns
   ...
   character, intent(in) :: rep
   real(dp), intent(inout) :: q(nf_), p(nf_), qe(ns), pe(ns)
   real(dp), intent(out) :: bt(nt+1,ns), Et(nt+1), Ead(nt+1)
   integer, intent(out) :: ierr
...
subroutine runpar_ead(q, p, qe, pe, bt, Et, Ead, ierr, rep, dt, nt, nf, ns, np)
   ...
   allocate(dbt(nt+1,ns,np), dEt(nt+1,np), dEad(nt+1,np))

!$omp parallel do default(shared) private(j)
   do j=1,np
      call runtrj_obs_ead(q(:,j), p(:,j), qe(:,j), pe(:,j), dbt(:,:,j), dEt(:,j), dEad(:,j), rep, dt, ierr(j), nt, nf, ns)

      if (ierr(j).ne.0) then
         ! match old runpar_ead behavior
         dbt(:,:,j) = 0.d0
         dEt(:,j)   = 0.d0
         dEad(:,j)  = 0.d0
      end if
